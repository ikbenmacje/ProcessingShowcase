...
void draw() 
{
  ...
  for ( float r=rMin; r<rMax; r+=rStep) {
    ...
    float ellipseSize = map(r,0,rMax,10, 25);

    for (float a=0; a<360; a+=360/aSegment) {
      colArrayCounter++;
      if(colArrayCounter > 5) colArrayCounter = 0;
      
      fill(colArray[colArrayCounter],random(50,200));
      noStroke();
      pushMatrix();
      rotate(radians(a));
  ...
  if(channel4note != 0){
    ...
    if(r == 0) fill(70,216,255);
    else if(r == 1) fill(255);
    else if(r == 2) fill(255,0,0);
